...
def test():
	...
	for url in f1.readlines()[:10]:
		...
		if id_ not in id_list:
			...
			while True:
				# try:
				r = requests.get('https://zhidao.baidu.com/mobile/replies?rn=6&new=1&hasLoadArgue=0&qid=984489770420233818&samp_hit=246&pn=1&deleteArgue=0',headers=headers,timeout=20,verify=False)
				requests.adapters.DEFAULT_RETRIES = 5
				s = requests.session()
				s.keep_alive = False
				print(len(r.content))
				# head = json.dumps(r.headers)
				# head = json.loads(head)
				# print(head['Content-Length'])
				sleep(10)
				# break
