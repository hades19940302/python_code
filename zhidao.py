...
def test():
	for i in range(700,750):
		...
		for en in entry:
			...
			with codecs.open('baiduzhidao_url_list.txt','a+','utf-8') as f :
				if url not in f.readlines():
					f.write(url+'\r\n')
					f.close()
				else:
					pass
			...
			# 			tmp = answer2.xpath('./following-sibling::div[1]/div/div[@class="question-meta-support question-meta-support-area"]/span/@data-support')[0]
