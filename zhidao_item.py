...
def test():
	...
	if id_ not in id_list:
		...
		for r_question_list_title in r_question_list_titles:
			...
			with codecs.open('zhidao_question_to_question_22.txt','a+','utf-8') as f1:
				try:
					if desc == '':
		...
		if len(answers)==0:
			...
			for answer1 in answers:
				...
				title = answer1.xpath('./div/div/div')[0].xpath('string(.)').strip()
				if tmp == [] or tmp[0]==0:
					pass
				else:
					like = tmp[0]
					if len(bingo)==0:
						...
						right = str(1)
					with codecs.open('zhidao_question_to_answer_22.txt','a+','utf-8') as f:
							if desc == '':
