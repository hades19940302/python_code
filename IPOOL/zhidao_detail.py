...
def test():
	f1 = open('qr_ids_not_in_ids_by_2018_04_20.txt','rb')
	flag = 1
	for line in f1.readlines():
		id_ = line.replace('\r','').replace('\n','').strip()
		url = 'https://zhidao.baidu.com/question/'+id_+'?fr=iks&ie=utf-8'
		...
		for i in range(5):
			...
			while True:
				try:
					r2 = requests.get(r2_url, headers=headers2, timeout=5, verify=False)
			...
			if selector2 == '' or selector2 == None:
				...
				if answers2 == [] :
					...
					for answer2 in answers2:
						like = answer2.xpath('./div[starts-with(@class,"append")]/div/div[@class="question-meta question-meta-support question-meta-support-area"]/span/@data-support')
						title = answer2.xpath('./div[@class="w-detail-full"]/div[@class="w-detail-container w-detail-"]/div/div')
						print(like)
						if title == []:
