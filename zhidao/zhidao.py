...
def test(xx):
    f1 = open('qr_urls_not_in_2.txt', 'rb')
    flag = 1
    for url in f1.readlines():
        ...
        if id_ not in id_list:
            ...
            r_question_list_titles = selector.xpath(
                '//div[@class="w-question-list"]/a/span[@class="r-question-list-title "]')
            ...
            if bingo == []:
                answers = selector.xpath(
                    '//div[@class="w-reply-container w-reply-item-normal"]/div[@class="w-detail-full"]/div[@class="w-detail-container w-detail-0"]/div/div')
                like = selector.xpath(
                    '//div[@class="w-reply-container w-reply-item-normal"]/div[@class="append"]/div/div[@class="question-meta-support question-meta-support-area"]/span/@data-support')
                if like == []:
                    like = selector.xpath('//div[starts-with(@class,"w-support ico-tmb enhance-sup")]/b/text()')
                    if like == []:
                        like = selector.xpath(
                            '//span[@class="js-question-meta-num question-meta-support-num"]/@data-support')
                        if like == []:
                            like = ['0']
                    ...
                    if like[0] == '0' or like[0] == 0:
                        ...
                        if answers == []:
                            ...
                            with codecs.open('zhidao_question_to_answer.txt', 'a+', 'utf-8') as f:
                                ...
                                if desc == '':
                                    s = ('1' + '\t' + 'qid:' + id_ + '\t' + question.xpath('string(.)').strip().replace(
                                        '\t', '') + '\t' + answers[0].xpath('string(.)').strip().replace('\t',
                                                                                                         '') + '\t' + '0' + '\t' + str(
                                        like[0])).strip().replace('\n', '').replace('\r', '').strip('\n')
                                    f.write(s + '\r\n')
                                    f.close()
                                else:
                                    s = ('1' + '\t' + 'qid:' + id_ + '\t' + question.xpath('string(.)').strip().replace(
                                        '\t', '') + '#' + desc.replace('\t', '') + '\t' + answers[0].xpath(
                                        'string(.)').strip().replace('\t', '') + '\t' + '0' + '\t' + str(
                                        like[0])).strip().replace('\n', '').replace('\r', '').strip('\n')
                                    f.write(s + '\r\n')
                                    f.close()
                ...
                answers = selector.xpath('//div[@class="full-content"]')
                like = selector.xpath(
                    '//div[starts-with(@class,"append")]/div/div[@class="question-meta-support question-meta-support-area"]/span/@data-support')
                if answers == [] or like == []:
                    answers = selector.xpath('//div[starts-with(@class,"full-content")]')
                    print(answers)
                    if like == []:
                        like = selector.xpath(
                            '//span[@class="js-question-meta-num question-meta-support-num"]/@data-support')
                ...
                with codecs.open('zhidao_question_to_answer.txt', 'a+', 'utf-8') as f:
                    if desc == '':
                        s = ('1' + '\t' + 'qid:' + id_ + '\t' + question.xpath('string(.)').strip().replace('\t',
                                                                                                            '') + '\t' +
                             answers[0].xpath('string(.)').strip().replace('\t', '') + '\t' + '1' + '\t' + str(
                                    like[0])).strip().replace('\n', '').replace('\r', '').strip('\n')
                        f.write(s + '\r\n')
                        f.close()
                    else:
            ...
            try:
                for r_question_list_title in r_question_list_titles:
                    ...
                    with codecs.open('zhidao_question_to_question.txt', 'a+', 'utf-8') as f1:
                        if desc == '':
                            s = ('1' + '\t' + 'qid:' + str(id_) + '\t' + question.xpath('string(.)').strip().replace(
                                '\t', '') + '\t' + r_question_list_title.xpath('string(.)').strip().replace('\t',
                                                                                                            '')).strip().replace(
                                '\n', '').replace('\r', '')
                            f1.write(s + '\r\n')
                            f1.close()
                            ...
                            s = ('1' + '\t' + 'qid:' + str(id_) + '\t' + question.xpath('string(.)').strip().replace(
                                '\t', '') + '#' + desc.replace('\t', '') + '\t' + r_question_list_title.xpath(
                                'string(.)').strip().replace('\t', '')).strip().replace('\n', '').replace('\r', '')
                            f1.write(s + '\r\n')
                            f1.close()
            ...
            answers2_list = []
            for i in range(5):
                r2_url = 'http://zhidao.baidu.com/mobile/replies?rn=6&new=1&hasLoadArgue=0&qid=' + id_ + '&samp_hit=246&pn=' + str(
                    i) + '&deleteArgue=0'
                headers2 = {
                    'Host': 'zhidao.baidu.com',
                ...
                if selector2 == '' or selector2 == None:
                    ...
                    if answers2 == []:
                        ...
                        for answer2 in answers2:
                            like = answer2.xpath(
                                './div[@class="append"]/div/div[@class="question-meta-support question-meta-support-area"]/span/@data-support')
                            title = answer2.xpath(
                                './div/div[@class="w-detail-full"]/div[@class="w-detail-container w-detail-"]/div/div')
                            if title == []:
                                            ...
                                            if like == []:
                                                ...
                                                if like[0] == 0 or like[0] == '0':
                                                    ...
                                                    try:
                                                        ...
                                                        with codecs.open('zhidao_question_to_answer.txt', 'a+',
                                                                         'utf-8') as f:
                                                            if desc == '':
                                                                s = ('1' + '\t' + 'qid:' + id_ + '\t' + question.xpath(
                                                                    'string(.)').strip().replace('\t', '') + '\t' +
                                                                     title[0].xpath('string(.)').strip().replace('\t',
                                                                                                                 '') + '\t' + '0' + '\t' + str(
                                                                            like[0])).strip().replace('\n', '').replace(
                                                                    '\r', '').strip('\n')
                                                                f.write(s + '\r\n')
                                        ...
                                        if like == []:
                                            ...
                                            if like[0] == 0 or like[0] == '0':
                                                ...
                                                with codecs.open('zhidao_question_to_answer.txt', 'a+', 'utf-8') as f:
                                                    if desc == '':
                                                        s = ('1' + '\t' + 'qid:' + id_ + '\t' + question.xpath(
                                                            'string(.)').strip().replace('\t', '') + '\t' + title[
                                                                 0].xpath('string(.)').strip().replace('\t',
                                                                                                       '') + '\t' + '0' + '\t' + str(
                                                            like[0])).strip().replace('\n', '').replace('\r', '').strip(
                                                            '\n')
                                                        f.write(s + '\r\n')
                                                        ...
                                                        s = ('1' + '\t' + 'qid:' + id_ + '\t' + question.xpath(
                                                            'string(.)').strip().replace('\t', '') + '#' + desc.replace(
                                                            '\t', '') + '\t' + title[0].xpath(
                                                            'string(.)').strip().replace('\t',
                                                                                         '') + '\t' + '0' + '\t' + str(
                                                            like[0])).strip().replace('\n', '').replace('\r', '').strip(
                                    ...
                                    if like == []:
                                        ...
                                        if like[0] == 0 or like[0] == '0':
                                            ...
                                            with codecs.open('zhidao_question_to_answer.txt', 'a+', 'utf-8') as f:
                                                ...
                                                if desc == '':
                                                    s = ('1' + '\t' + 'qid:' + id_ + '\t' + question.xpath(
                                                        'string(.)').strip().replace('\t', '') + '\t' + title[0].xpath(
                                                        'string(.)').strip().replace('\t',
                                                                                     '') + '\t' + '0' + '\t' + str(
                                                        like[0])).strip().replace('\n', '').replace('\r', '').strip(
                                                        '\n')
